...
op_stop() {
        nixops ssh-for-each --parallel 'systemctl stop cardano-node 2>/dev/null || true; systemctl stop systemd-journald 2>/dev/null || true; systemctl stop cardano-tracer 2>/dev/null || true' &
        wait
}
