...
class RelayUpdate
  ...
  def initialize(@refresh : Bool, @allOpt : Bool, @edgeOpt : Bool, @relOpt : Bool, @extraDeployFlagsOpt : String,  @minOpt : Int32,
    ...
    if (@emailOpt != "")
      if runCmdSecret("nix eval --impure --raw --expr '(import #{PATH_MOD}/static/ses.nix).sesSmtp.username'").success?
        @sesUsername = IO_CMD_OUT.to_s
      else
        abort("Unable to process the ses username.")
      end

      if runCmdSecret("nix eval --impure --raw --expr '(import #{PATH_MOD}/static/ses.nix).sesSmtp.secret'").success?
        @sesSecret = IO_CMD_OUT.to_s
      else
    ...
    if runCmdVerbose("nix eval --impure --raw --expr '(import #{PATH_MOD}/nix {}).globals.environmentName'").success?
      @cluster = IO_CMD_OUT.to_s
    else
      updateAbort("Unable to process the environment name from the globals file.")
    end

    if runCmdVerbose("nix eval --impure --raw --expr '(import #{PATH_MOD}/nix {}).globals.deploymentName'").success?
      @deployment = IO_CMD_OUT.to_s
    else
