...
let
  ...
  unstableNixpkgs = import defaultSourcePaths.nixpkgs-unstable {};
  ...
  oauthEnv = self: super: {
    inherit (unstableNixpkgs) buildGoModule;
    oauth2_proxy = self.callPackage ../pkgs/oauth2_proxy {};
  };

  oauthEnvIsolated = self: super: {
    inherit (self.extend oauthEnv) oauth2_proxy;
  };
