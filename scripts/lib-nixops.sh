...
nixops_deploy() {
        local include="$1"
        local deploylog="$2"
        local node_rev=$(jq --raw-output '.["cardano-node"].rev' nix/sources.json | cut -c-8)
        local dbsync_rev=$(jq --raw-output '.["cardano-db-sync"].rev' nix/sources.json | cut -c-8)
        local ops_rev=$(git rev-parse HEAD | cut -c-8)
        local ops_branch=$(git symbolic-ref --short HEAD)
        local ops_checkout_state=$(if git diff --quiet --exit-code
        ...
        if ! nixops deploy --max-concurrent-copy 50 -j 4 ${include} \
                 >"${deploylog}" 2>&1
